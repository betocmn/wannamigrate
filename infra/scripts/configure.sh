...
echo "--------------------------"
cp /wanna/infra/config/wsgi.conf /etc/apache2/mods-enabled/wsgi.conf
cp /wanna/infra/config/wannamigrate.conf /etc/apache2/sites-enabled/wannamigrate.conf
...
echo "   => mysql -u root -p"
echo "3. Activate your virtualenv and run a 'migrate' inside your django application."
echo "   => source /wannavenv/bin/activate"
echo "   => python /wanna/app/wannamigrate/manage.py migrate"
