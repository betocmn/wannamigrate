...
/**
 *  Display global status error
 *
 */
function display_error_message( msg, can_close ) {
	$( "#global_alert_error" ).show();
	$( "#global_alert_error_msg" ).html( msg );
    if ( !can_close ){
        $( "#global_alert_error_close" ).show();
    } else {
        $( "#global_alert_error_close" ).hide();
    }
	window.scrollTo( 0, 0 );
}

/**
 *  Display global status success
 *
 */
function display_success_message( msg, can_close ) {
	$( "#global_alert_success" ).show();
	$( "#global_alert_success_msg" ).html( msg );
    if ( !can_close ){
        $( "#global_alert_success_close" ).show();
    } else {
        $( "#global_alert_success_close" ).hide();
    }
	window.scrollTo( 0, 0 );
}
